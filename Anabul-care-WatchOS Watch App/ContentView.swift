...
struct WatchPetDashboardView: View {
            ...
            VStack(spacing: 12) {
                ...
                VStack(spacing: 8) {
                    ForEach(dailyTasks) { dailyTask in
                        WatchTaskRow(pet: pet, dailyTask: dailyTask)
                    }
                }
                .padding(.bottom, 10)
            }
    ...
    private var completedCount: Int {
        ...
        return dailyTasks.filter { dailyTask in
            pet.activities.contains { $0.type == dailyTask.type.rawValue && Calendar.current.isDate($0.timestamp, inSameDayAs: today) }
        }.count
    }
}
...
struct WatchTaskRow: View {
    ...
    var isLogged: Bool {
        let today = Calendar.current.startOfDay(for: Date())
        return pet.activities.contains { $0.type == dailyTask.type.rawValue && Calendar.current.isDate($0.timestamp, inSameDayAs: today) }
    }
    ...
    var body: some View {
        Button {
            ...
            HStack(spacing: 10) {
                Image(systemName: dailyTask.icon)
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text(dailyTask.timeRecommendation)
                        .font(.system(size: 10))
                        .foregroundColor(.secondary)
                }
    ...
    private func toggleActivity() {
        ...
        withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
            if isLogged {
                if let index = pet.activities.firstIndex(where: { $0.type == dailyTask.type.rawValue && Calendar.current.isDate($0.timestamp, inSameDayAs: today) }) {
